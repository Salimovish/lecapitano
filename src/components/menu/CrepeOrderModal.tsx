...
interface CrepeOrderModalProps {
  crepe: Crepe;
  isOpen: boolean;
  onClose: () => void;
}

export default function CrepeOrderModal({ crepe, isOpen, onClose }: CrepeOrderModalProps) {
  const { addToCart } = useCart();
  const [quantity, setQuantity] = useState(1);
  const [selectedSize] = useState<'small' | 'medium' | 'large'>('medium');

  // Vérifier si le prix est défini et valide
  const price = typeof crepe?.base_price === 'number' ? crepe.base_price : 0;
            ...
            >
              <div className="p-6">
                <div className="relative">
                  ...
                  <h2 className="text-2xl font-bold mb-4">{crepe.name}</h2>
                  ...
                  {crepe.ingredients && (
                    <div className="mb-4">
                      ...
                      <p className="text-gray-600">{crepe.ingredients.join(', ')}</p>
                    </div>
                  )}
